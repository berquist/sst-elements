...
bool GOBLINHMCSimBackend::issueCustomRequest(ReqId reqId, Interfaces::StandardMem::CustomData* data) { 
    ...
    if( MapCmd == NULL ){
        ...
        for (auto itr = CmdMapping.begin(); itr != CmdMapping.end(); ++itr){
            MapCmd = *itr;
            str << "Entry " << num << ": " << MapCmd->getOpcode() << " " << MapCmd->getSrcSize() << " " << MapCmd->getSrcType() << " " << MapCmd->getTargetType() << std::endl;
            num++;
        }
        output->output(CALL_INFO, "%s", str.str().c_str());
        output->fatal(CALL_INFO, -1, "%s, Error: Translation for opcode (%" PRIu32 "), size (%u) was not found.\n",
            getName().c_str(), cmd, numBytes);
        return false;
    }
