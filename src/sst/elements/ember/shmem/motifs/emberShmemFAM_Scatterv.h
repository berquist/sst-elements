        ...
        case Work:
			...
			if ( m_blockStride ) {
				if ( m_blocking ) {
        			...
        			enQ_fam_scatter_nonblocking( evQ, m_src, m_fd, m_numBlocks, m_firstBlock, m_blockStride, m_blockSize );
				}
			} else {
				if ( m_blocking ) {
        			enQ_fam_scatterv_blocking( evQ, m_src, m_fd, m_numBlocks, m_indexes, m_blockSize );
				} else {
